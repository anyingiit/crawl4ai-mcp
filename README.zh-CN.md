...
<!-- translation-of: README.md sha256:382d1d599dfd4a43 -->
...
`crawl4ai-setup` 会下载 `crawl4ai` 所驱动的 Chromium；只有当设置了
`CRAWL4AI_MODE=api` 且不会运行任何依赖浏览器的层时才可以跳过它。

若想在 Linux 上以后台服务方式运行，先从 `.env.example` 创建 `.env` 并将其权限
收紧为仅所有者可读写——即使未启用任何付费服务商，`scripts/install-user-service.sh`
也要求如此：
