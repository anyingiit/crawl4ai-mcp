...
- 代理池凭据按池注入：`WEBSHARE_PROXY_USERNAME`/`WEBSHARE_PROXY_PASSWORD` 与 `OXYLABS_PROXY_USERNAME`/`OXYLABS_PROXY_PASSWORD`（见 `.env.example`），两池互不混淆；用户/密码只填一个即配置错误，服务启动时直接拒绝（`load_config` 校验报错），认证 CONNECT 携带正确的 `Proxy-Authorization`
